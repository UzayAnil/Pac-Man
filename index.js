...
window.onload = function () {
    ...
    document.getElementById("page").style.transform = "scale(" + ((window.screen.availWidth * 780 > window.screen.availHeight * 620) ? window.screen.availHeight / 780 : window.screen.availWidth / 620) + ")"; //resizes page to fit any screen
    ...
    //variables
    let timerRunning = false, framesElapsed = 0;
    let offsetTop = 42, offsetLeft = 2, overlap = 6, size = 22;
    let score = 0, level = 1, lives = 3, dotsEaten = 0, dotsCounter = 0, ghostsEaten = 0, dotsBeforeExit = [], sincePacmanLastAte = 0;
    let audioOn = true;
    ...
    class ghost {
        constructor(name, dir, pos, target, dotsBeforeExit, setTarget) {
            ...
            this.switchModes = new periodicFunction(function () {
                ...
                if (ghost.switchCounter < 8) { //stops at the fourth chase mode
            ...
            this.frightened = function () {
                if (this.chaseOrRun == false && this.mode != 'invisible' && this.mode != '') {
                    ...
                    } else if (this.frightenedModeCounter > 0 && this.frightenedModeCounter < this.frightenedModeTimeout) {
                        this.frightenedModeCounter++;
                    } else {
                        this.chaseOrRun = true;
            ...
            this.findWayOut = new periodicFunction(function () {
                if (ghost.started == true) {
                    ...
                    document.documentElement.style.setProperty('--' + ghost.name + 'Transition', '0.125s');
            ...
            this.moveGhost = new periodicFunction(function () {
                if (ghost.isMoving) {
                    if (document.getElementById(ghost.name).style.backgroundColor == 'transparent') { //error handling. Updates the ghost's target if he's lost when rushing back to the house
                        ghost.mode = 'invisible';
                        ghost.target.x = 14;
                        ghost.target.y = 11;
                    }
                    if (ghost.pos.y == 14 && ((ghost.pos.x > 0 && ghost.pos.x <= 5) || (ghost.pos.x < 27 && ghost.pos.x >= 22))) {
                        ghost.speed = 10;
                        document.documentElement.style.setProperty('--' + ghost.name + 'Transition', '0.25s');
                    } else if (ghost.pos.y == 14 && (ghost.pos.x == 0 || ghost.pos.x == 27)) {
                        document.documentElement.style.setProperty('--' + ghost.name + 'Transition', '0s');
                    } else {
                        if (ghost.mode == 'frightened') {
                            ghost.speed = 8;
                            document.documentElement.style.setProperty('--' + ghost.name + 'Transition', '0.2s');
                        } else if (ghost.mode == 'chase' || ghost.mode == 'scatter') {
                            ghost.speed = 5;
                            document.documentElement.style.setProperty('--' + ghost.name + 'Transition', '0.125s');
                        } else if (ghost.mode == 'invisible') {
                            ghost.speed = 2;
                            document.documentElement.style.setProperty('--' + ghost.name + 'Transition', '0.05s');
                        }
                    }
                    if (ghost.pos.x == 0 && ghost.pos.y == 14 && ghost.dir == 'left') {
                        //makes it possible for ghosts to go through the left tunnel
                        document.documentElement.style.setProperty('--' + ghost.name + 'Transition', '0s'); //makes the ghost disappear into nowhere
                        ghost.pos.x = 26; //and come back through the right tunnel
                    } else if (ghost.pos.x == 27 && ghost.pos.y == 14 && ghost.dir == 'right') {
                        ...
                        if (maze[ghost.pos.y + convertDir(ghost.dir).y][ghost.pos.x + convertDir(ghost.dir).x] != 0) {
                            ...
                            ghost.dir = next;
                        } else { //error handling. Makes the ghost pick any free tile if he's stuck
            ...
            this.collide = function () {
                if (this.pos.x == pacman.pos.x && this.pos.y == pacman.pos.y) {
                    if (this.mode == 'frightened') {
                        ...
                        document.getElementById(this.name + 'Score').innerText = 100 * Math.pow(2, ghostsEaten);
                        document.documentElement.style.setProperty('--pacmanColor', 'transparent');
                        siren.setVolume(0);
                        ...
                        setTimeout(function () {
                            ...
                            document.documentElement.style.setProperty('--pacmanColor', '#FFFD38');
                        }, 1000);
                    ...
                    if (this.mode == 'chase' || this.mode == 'scatter') {
                        ...
                        lives--;
                        if (lives == 2) {
                            document.documentElement.style.setProperty('--life2', 'transparent');
                        } else if (lives == 1) {
                            document.documentElement.style.setProperty('--life1', 'transparent');
                        }
                        setTimeout(function () {
            ...
            this.reverse = function () {
                let opposite = '', straightPath = (maze[this.pos.y + 1][this.pos.x] != 0 && maze[this.pos.y - 1][this.pos.x] != 0 && maze[this.pos.y][this.pos.x + 1] == 0 && maze[this.pos.y + 1][this.pos.x + 1] == 0 && maze[this.pos.y - 1][this.pos.x + 1] == 0 && maze[this.pos.y][this.pos.x - 1] == 0 && maze[this.pos.y + 1][this.pos.x - 1] == 0 && maze[this.pos.y - 1][this.pos.x - 1] == 0) || (maze[this.pos.y + 1][this.pos.x] == 0 && maze[this.pos.y + 1][this.pos.x - 1] == 0 && maze[this.pos.y + 1][this.pos.x + 1] == 0 && maze[this.pos.y - 1][this.pos.x] == 0 && maze[this.pos.y - 1][this.pos.x - 1] == 0 && maze[this.pos.y - 1][this.pos.x + 1] == 0 && maze[this.pos.y][this.pos.x + 1] != 0 && maze[this.pos.y][this.pos.x - 1] != 0);
                switch (this.dir) {
                    case 'left':
                        opposite = 'right';
                        break;
                ...
                if (this.reverseDir && straightPath) { //waits for the ghost to enter a straight path before attempting to reverse. Trying this on junctions brought nasty bugs that I was unable to track :(
                    this.dir = opposite;
                    this.reverseDir = false;
                }
            };
    ...
    let blinky = new ghost('blinky', 'right', { x: 14, y: 11 }, { x: 25, y: -2 }, 0, function () {
        ...
        if (this.switchCounter == 1 && this.mode == 'scatter') {
            ...
            if (this.mode == 'chase' || this.mode == 'scatter' || this.mode == 'frightened') {
    ...
    let pinky = new ghost('pinky', 'left', { x: 14, y: 14 }, { x: 2, y: -2 }, 0, function () {
    ...
    let inky = new ghost('inky', 'right', { x: 12, y: 14 }, { x: 27, y: 33 }, 30, function () {
        //inky stays around blinky and pacman
        if (this.mode == 'chase' || this.mode == 'frightened') {
            this.target.x = blinky.pos.x + (((pacman.pos.x + 2 * convertDir(pacman.dir).x) - blinky.pos.x) * 2);
            this.target.y = blinky.pos.y + (((pacman.pos.y + 2 * convertDir(pacman.dir).y) - blinky.pos.y) * 2);
        } else if (this.mode == 'scatter') {
            this.target.x = 27;
            this.target.y = 33;
        } else if (this.mode == 'invisible') {
    ...
    let clyde = new ghost('clyde', 'left', { x: 16, y: 14 }, { x: 0, y: 33 }, 30, function () {
        //clyde only chases pacman when he is more than eight tiles away
        if (this.mode == 'chase' || this.mode == 'frightened') {
            if ((Math.abs(pacman.pos.x - this.pos.x) > 8) || (Math.abs(pacman.pos.y - this.pos.y) > 8)) {
                this.target.x = pacman.pos.x;
                this.target.y = pacman.pos.y;
            } else {
                this.target.x = 0;
    ...
    let intro, newGame, siren, invincible, regenerating, eatDots, eatGhost, playerDeath; //sounds
    function finishedLoading(bufferList) {
        hud.innerText = 'READY!';
        //initializes audio
        intro = new gameAudio('intro', bufferList[0], function () {
            hud.innerText = '';
        ...
        newGame = new gameAudio('cutscene', bufferList[1], function () {
            hud.innerText = '';
            document.documentElement.style.setProperty('--pacmanAnimation', '0.2s');
            timerRunning = true;
            audioOn = true;
    ...
    let timeout = null, currentFrame = 0;
    function updateDots() {
        if (maze[pacman.pos.y][pacman.pos.x] == 2) {
            ...
            maze[pacman.pos.y][pacman.pos.x] = 1;
            ctx.beginPath();
            ctx.arc(size * pacman.pos.x + 11, size * pacman.pos.y + 11, 9, 0, 2 * Math.PI);
            ctx.fillStyle = '#000';
            ctx.fill();
            ctx.closePath();
        } else {
        ...
        if (dotsCounter == 244) { //increases the level when the maze is cleared
            levelUp();
        }
        if (framesElapsed == currentFrame) {
            if ((dotsEaten == 70 || dotsEaten == 170)) { //adds bonus fruits when pacman eats 70 or 170 dots
                bonus.style.top = '410px';
                bonus.style.left = '304px';
                timeout = setTimeout(function () { //makes them disappear after 9 seconds
                    bonus.style.top = '730px';
                    bonus.style.left = '580px';
                }, 9000);
            }
        }
        if (pacman.pos.x == 14 && pacman.pos.y == 17 && bonus.style.top == '410px' && bonus.style.left == '304px' && timeout != null) { //increases score by 100 when pacman eats the cherries
    ...
    //moves pacman
    let movePlayer = new periodicFunction(function () {   //an object with a function and its counter
        ...
        if (pacman.pos.x == 0 && pacman.pos.y == 14 & pacman.dir == 'left') {
        ...
        } else if (pacman.pos.x == 27 && pacman.pos.y == 14 & pacman.dir == 'right') {
            ...
            if (maze[pacman.pos.y + convertDir(pacman.dir).y][pacman.pos.x + convertDir(pacman.dir).x] != 0) { //checks if a path lies ahead
        ...
        pacmanGraphic.style.left = (offsetLeft + pacman.pos.x * size - overlap) + 'px';
    ...
    function pathFinder(ghost, minOrMax) {
        ...
        switch (ghost.dir) {
            ...
            case 'up':
                up = ((maze[ghost.pos.y - 2][ghost.pos.x] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - ghost.pos.x, 2) + Math.pow(ghost.target.y - (ghost.pos.y - 2), 2)));
                left = ((maze[ghost.pos.y - 1][ghost.pos.x - 1] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x - 1), 2) + Math.pow(ghost.target.y - (ghost.pos.y - 1), 2)));
                down = notAnOption;
                right = ((maze[ghost.pos.y - 1][ghost.pos.x + 1] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x + 1), 2) + Math.pow(ghost.target.y - (ghost.pos.y - 1), 2)));
                break;
            case 'left':
                up = ((ghost.pos.x - 1 == 12 && (ghost.pos.y == 11 || ghost.pos.y == 23)) || (ghost.pos.x - 1 == 15 && (ghost.pos.y == 11 || ghost.pos.y == 23))) ? notAnOption : ((maze[ghost.pos.y - 1][ghost.pos.x - 1] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x - 1), 2) + Math.pow(ghost.target.y - (ghost.pos.y - 1), 2)));
                left = ((maze[ghost.pos.y][ghost.pos.x - 2] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x - 2), 2) + Math.pow(ghost.target.y - ghost.pos.y, 2)));
                down = ((maze[ghost.pos.y + 1][ghost.pos.x - 1] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x - 1), 2) + Math.pow(ghost.target.y - (ghost.pos.y + 1), 2)));
                right = notAnOption;
                break;
            case 'down':
                up = notAnOption;
                left = ((maze[ghost.pos.y + 1][ghost.pos.x - 1] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x - 1), 2) + Math.pow(ghost.target.y - (ghost.pos.y + 1), 2)));
                down = ((maze[ghost.pos.y + 2][ghost.pos.x] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - ghost.pos.x, 2) + Math.pow(ghost.target.y - (ghost.pos.y + 2), 2)));
                right = ((maze[ghost.pos.y + 1][ghost.pos.x + 1] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x + 1), 2) + Math.pow(ghost.target.y - (ghost.pos.y + 1), 2)));
                break;
            case 'right':
                up = ((ghost.pos.x + 1 == 12 && (ghost.pos.y == 11 || ghost.pos.y == 23)) || (ghost.pos.x + 1 == 15 && (ghost.pos.y == 11 || ghost.pos.y == 23))) ? notAnOption : ((maze[ghost.pos.y - 1][ghost.pos.x + 1] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x + 1), 2) + Math.pow(ghost.target.y - (ghost.pos.y - 1), 2)));
                left = notAnOption;
                down = ((maze[ghost.pos.y + 1][ghost.pos.x + 1] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x + 1), 2) + Math.pow(ghost.target.y - (ghost.pos.y + 1), 2)));
                right = ((maze[ghost.pos.y][ghost.pos.x + 2] == 0) ? notAnOption : Math.sqrt(Math.pow(ghost.target.x - (ghost.pos.x + 2), 2) + Math.pow(ghost.target.y - ghost.pos.y, 2)));
                break;
        }
    ...
    function updateGhostGraphic(ghost) {
        ...
        switch (ghost.name) {
            case 'inky':
                inkyGraphic.style.left = (offsetLeft + ghost.pos.x * size - overlap) + 'px';
                inkyGraphic.style.top = (offsetTop + ghost.pos.y * size - overlap) + 'px';
                break;
            case 'blinky':
                blinkyGraphic.style.left = (offsetLeft + ghost.pos.x * size - overlap) + 'px';
                blinkyGraphic.style.top = (offsetTop + ghost.pos.y * size - overlap) + 'px';
                break;
            case 'pinky':
                pinkyGraphic.style.left = (offsetLeft + ghost.pos.x * size - overlap) + 'px';
                pinkyGraphic.style.top = (offsetTop + ghost.pos.y * size - overlap) + 'px';
                break;
            case 'clyde':
                clydeGraphic.style.left = (offsetLeft + ghost.pos.x * size - overlap) + 'px';
                clydeGraphic.style.top = (offsetTop + ghost.pos.y * size - overlap) + 'px';
                break;
        }
    ...
    function updateAudio() {
        if (audioOn) {
            if (blinky.mode == 'invisible' || pinky.mode == 'invisible' || inky.mode == 'invisible' || clyde.mode == 'invisible') {
                siren.setVolume(0);
                invincible.setVolume(0);
                regenerating.setVolume(1);
            } else if (blinky.mode == 'frightened' || pinky.mode == 'frightened' || inky.mode == 'frightened' || clyde.mode == 'frightened') {
                siren.setVolume(0);
    ...
    function initialize() {
        ...
        blinky = new ghost('blinky', 'right', { x: 14, y: 11 }, { x: 25, y: -2 }, dotsBeforeExit[0][((level > 3) ? 3 : level) - 1], function () {
            if (this.switchCounter == 1 && this.mode == 'scatter') {
                this.target.x = 25;
                this.target.y = -2;
            } else {
        ...
        pinky = new ghost('pinky', 'left', { x: 14, y: 14 }, { x: 2, y: -2 }, dotsBeforeExit[1][((level > 3) ? 3 : level) - 1], function () {
            if (this.mode == 'chase' || this.mode == 'frightened') {
                this.target.x = pacman.pos.x + 4 * convertDir(pacman.dir).x;
                this.target.y = pacman.pos.y + 4 * convertDir(pacman.dir).y;
            } else if (this.mode == 'scatter') {
        ...
        inky = new ghost('inky', 'right', { x: 12, y: 14 }, { x: 27, y: 33 }, dotsBeforeExit[2][((level > 3) ? 3 : level) - 1], function () {
            if (this.mode == 'chase' || this.mode == 'frightened') {
                this.target.x = blinky.pos.x + (((pacman.pos.x + 2 * convertDir(pacman.dir).x) - blinky.pos.x) * 2);
                this.target.y = blinky.pos.y + (((pacman.pos.y + 2 * convertDir(pacman.dir).y) - blinky.pos.y) * 2);
            } else if (this.mode == 'scatter') {
        ...
        clyde = new ghost('clyde', 'left', { x: 16, y: 14 }, { x: 0, y: 33 }, dotsBeforeExit[3][((level > 3) ? 3 : level) - 1], function () {
            if (this.mode == 'chase' || this.mode == 'frightened') {
                if ((Math.abs(pacman.pos.x - this.pos.x) > 8) || (Math.abs(pacman.pos.y - this.pos.y) > 8)) {
                    this.target.x = pacman.pos.x;
                    this.target.y = pacman.pos.y;
    ...
    let stop = false, frameCount = 0, fpsInterval, startTime, now, then, elapsed;
